...
def read_single_sequence(folder, seq_name, fps=25):
    ...
    for subject in tqdm(subjects):
        ...
        subject_path = osp.join(folder, subject)

        if not os.path.isdir(subject_path):
            continue

        actions = [x for x in os.listdir(subject_path) if x.endswith('.npz')]
        ...
        for action in actions:
            ...
            if 'mocap_frame_rate' not in data or 'poses' not in data:
                print(f"Skipping {fname} as it doesn't contain necessary keys.")
                continue

...
class VIBE(nn.Module):
    def __init__(
        ...
        # changed by Xiao Cui:add resnet-50
        #self.resnet = spin.get_part_pretrained_hmr5()
        self.resnet = spin.get_pretrained_hmr()
    ...
    def forward(self, input, J_regressor=None):
        # input size NTF, F = 5,224,224
        batch_size, seqlen = input.shape[:2]

        feature = input.view(batch_size * seqlen, 3, 224, 224).float()
        feature = self.resnet.feature_extractor(feature)
        feature = feature.view(batch_size, seqlen, 2048)
...
class VIBE_Demo(nn.Module):
    ...
    def forward(self, input, J_regressor=None):
        ...
        feature = self.resnet.feature_extractor(input.reshape(-1, nc, h, w))

...
def read_data(folder):
    ...
    for fname in tqdm(file_names):
        ...
        for img_i in imgs:
            ...
            flow_dir = osp.dirname(img_i)
            flow_path = osp.join(flow_dir, flow_filename)
            try:
                _ = np.load(flow_path)
        ...
        #features = extract_features(model, np.array(imgs) , bbox, dataset='pennaction', debug.sh.py=False)

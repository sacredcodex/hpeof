...
class Trainer():
    def __init__(
        ...
        if self.train_3d_loader:
            #self.train_3d_iter = iter(self.valid_loader)
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            if target_2d and target_3d:
                inp = torch.cat((target_2d['feature'], target_3d['feature']), dim=0).to(self.device)

            elif target_3d:
                inp = target_3d['feature'].to(self.device)
            else:
                inp = target_2d['feature'].to(self.device)
            ...
            '''
            if target_2d:
                torch.save(target_2d, '/data/anu_hpeof/vibe-flow/VIBE/output/t2d.pt')
            if target_3d:
                torch.save(target_3d, '/data/anu_hpeof/vibe-flow/VIBE/output/t3d.pt')

            inp = inp[:, :, :3, :, :]
            preds = self.generator(inp)
            torch.save(preds, '/data/anu_hpeof/vibe-flow/VIBE/output/out.pt')
            ...
            torch.save(loss_dict, '/data/anu_hpeof/vibe-flow/VIBE/output/loss.pt')
            print('save input and output', flush=True)
            import sys
            sys.exit()
            #'''

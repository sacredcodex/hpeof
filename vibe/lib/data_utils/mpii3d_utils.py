import sys
sys.path.append('/data/anu_hpeof/vibe-flow/VIBE')
import os
        ...
        for seq_i in seq_list:
            ...
            for j, vid_i in enumerate(vid_list):
                ...
                imgs_path = os.path.join(dataset_path,
                                         f'video_{user_i}_{seq_i}_{vid_i}')
                # per frame
                ...
                for i, img_i in tqdm_enumerate(img_list):
                    ...
                    image_filename = os.path.basename(img_i)
                    image_number = image_filename.split('.')[0]
                    image_number = int(image_number)
                    prev_image_number = image_number - 1
                    flow_filename = f"{prev_image_number}_{image_number}.npy"
                    flow_dir = os.path.dirname(img_i)
...
def read_test_data(dataset_path):
    ...
    for user_i in user_list:
        ...
        for frame_i, valid_i in tqdm(enumerate(valid)):
            ...
            img_i = os.path.join('mpi_inf_3dhp_test_set/mpi_inf_3dhp_test_set',
                                    'TS' + str(user_i),
                                    'imageSequence',
            ...
            image_filename = osp.basename(img_i)
            image_number = image_filename.split('_')[1].split('.')[0]
            image_number = int(image_number)
            prev_image_number = image_number - 1
            flow_filename = f"{prev_image_number}_{image_number}.npy"
            flow_path = os.path.join(dataset_path,
                                     'mpi_inf_3dhp_test_set/mpi_inf_3dhp_test_set',
                                    'TS' + str(user_i),
                                    'imageSequence',
                                     flow_filename)

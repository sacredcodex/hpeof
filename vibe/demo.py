...
def main(args):
    ...
    print('load video complete',flush=True)
    ...
    pretrained_file = download_ckpt(mine=True)
    ckpt = torch.load(pretrained_file)
    print(f'Performance of pretrained model on 3DPW: {ckpt["performance"]}')
    ckpt = ckpt['gen_state_dict']
    model.load_state_dict(ckpt, strict=False)
    model.eval()
    print(f'Loaded pretrained weights from \"{pretrained_file}\"',flush=True)
    ...
    print(f'Saving output results to \"{os.path.join(output_path, "vibe_output.pkl")}\".', flush=True)
